...
class WebBrowser(QMainWindow):    
    ...
    def initializeUI(self):    
        self.resize(764, 567)      
        self.setWindowTitle("Bluder Browser")        
        self.setWindowIcon(QIcon(os.path.join('icons', 'globe.svg')))
    ...
    def createToolbar(self):        
        ...
        back_page_button = QAction(QIcon(os.path.join('icons',  'arrow-left-circle.svg')), "Back", self)        
        back_page_button.triggered.connect(self.backPageButton)
        forward_page_button = QAction(QIcon(os.path.join('icons',  'arrow-right-circle.svg')), "Forward", self)        
        forward_page_button.triggered.connect(self.forwardPageButton)
        refresh_button = QAction(QIcon(os.path.join('icons',  'refresh-cw.svg')), "Refresh", self)        
        refresh_button.triggered.connect(self.refreshButton)
        home_button = QAction(QIcon(os.path.join('icons', 'home.svg')), "Home", self)        
        home_button.triggered.connect(self.homeButton)
        stop_button = QAction(QIcon(os.path.join('icons', 'x-circle.svg')), "Stop", self)        
        ...
        self.address_line.addAction(QIcon('icons/search.svg'), QLineEdit. LeadingPosition)        
